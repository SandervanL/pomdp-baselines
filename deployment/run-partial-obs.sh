...
#SBATCH --job-name="run-partial-obs"
...
##SBATCH --mem-per-gpu=10GB	  # only if requesting gpus, also mutually exclusive with --mem and --mem-per-cpu
...
# Setup modules
module load 2022r2	# main module, top of the module heirarchy
...
srun python /home/sajvanleeuwen/code/src/main.py \
  ...
  --env test-t-fully-v0 test-bend-fully-v0 test-cross-fully-v0 test-t-partial-v0 test-bend-partial-v0 test-cross-partial-v0 \
  --executions 1 \
  ...
  --max-episodes 1000 \
  --save-dir /home/sajvanleeuwen/logs/partial_obs_test
